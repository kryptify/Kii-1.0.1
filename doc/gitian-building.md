...
*Setup instructions for a Gitian build of KII Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the KII
...
- [Building KII Core](#building-kii-core)
...
Clone the git repositories for KII Core and Gitian.
...
in the KII Core repository under 'Fetch and create inputs' to install sources which require
...
To build KII Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the KII Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kii/.git/
...
./bin/gbuild --commit kii=${COMMIT} --url kii=${URL} ../kii/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kii git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kii/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kii-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kii-win-build.assert
