...
const char * const BITCOIN_CONF_FILENAME = "kii.conf";
const char * const BITCOIN_PID_FILENAME = "kiid.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "kiid -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KII
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KII
    // Mac: ~/Library/Application Support/KII
    // Unix: ~/.kii
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KII";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KII";
#else
    // Unix
    return pathRet / ".kii";
#endif
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty kii.conf if it does not excist
